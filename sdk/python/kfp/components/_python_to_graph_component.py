...
from kfp.components._naming import _convert_name_and_make_it_unique_by_adding_number, generate_unique_name_conversion_table, _sanitize_python_function_name, _convert_to_human_name
...
def create_graph_component_from_pipeline_func(pipeline_func) -> ComponentSpec:
    ...
    for output_name, output_value in graph_output_value_map.items():
        if not isinstance(output_value, TaskOutputArgument):
            raise TypeError('Only TaskOutputArgument instances should be returned from graph component, but got "{}".'.format(output_value))

    graph_output_specs = [OutputSpec(name=output_name, type=output_value.task_output._type) for output_name, output_value in graph_output_value_map.items()]
